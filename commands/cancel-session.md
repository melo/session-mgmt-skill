...
```bash
python3 ~/.claude/skills/session-mgmt-skill/scripts/backlog_ops.py update-status \
  "$BACKLOG_ITEM_ID" --status open --repo-root "$REPO_ROOT"
```
...
```bash
PRUNE_ROOT="${MAIN_WORKSPACE:-$REPO_ROOT}"
python3 ~/.claude/skills/session-mgmt-skill/scripts/session_prune.py --repo-root "$PRUNE_ROOT"
```
